...
ckpt_path = "/home/haoming/Bagel/experiments/results_tune/checkpoints/0009000"


# for name, module in model.named_modules():
#     for param_name, param in list(module.named_parameters(recurse=False)):
#         full_name = f"{name}.{param_name}" if name else param_name
#         if 'connector' in full_name and param.is_meta:
#             print(f"Replacing meta parameter: {full_name}")
#             with torch.no_grad():
#                 # Create real tensor
#                 new_tensor = torch.empty(param.shape, dtype=param.dtype, device='cpu')
#                 new_tensor.normal_(0, 0.02)
#                 # Replace the entire parameter
#                 setattr(module, param_name, torch.nn.Parameter(new_tensor))
if args.mode == 1:
    ...
    model = load_and_quantize_model(
        model, 
        weights_location=os.path.join(ckpt_path, "ema.safetensors"), 
...
def text_to_image(prompt, show_thinking=False, cfg_text_scale=4.0, cfg_interval=0.4, 
    ...
    result = inferencer(text=prompt, think=show_thinking,use_vilex=False, **inference_hyper)
...
def image_understanding(image: Image.Image, prompt: str, show_thinking=False, 
    ...
    result = inferencer(image=image, text=prompt, think=show_thinking, use_vilex=False,
...
def edit_image(image: Image.Image, prompt: str, show_thinking=False, cfg_text_scale=4.0, 
    ...
    result = inferencer(image=image, text=prompt, think=show_thinking,use_vilex=True, **inference_hyper)
...
if __name__ == "__main__": 
    # Save folder for evaluation outputs
    out_dir = "/home/haoming/Bagel/results/eval"
    os.makedirs(out_dir, exist_ok=True)

    # helper to ensure PIL Image
    def ensure_pil(image):
        ...
        if isinstance(image, torch.Tensor):
            ...
            if t.ndim == 4:
                t = t[0]
            # assume tensor in [0,1] with shape (C,H,W) or (H,W) etc.
    ...
    puppy_img = Image.open(puppy_path)
    edited, edit_thinking = edit_image(puppy_img, "in a dessert", seed=42)
    edited_pil = ensure_pil(edited)
    edit_path = os.path.join(out_dir, "puppy_edit.png")
    edited_pil.save(edit_path)
    with open(os.path.join(out_dir, "puppy_edit_thinking.txt"), "w") as fh:
        fh.write(edit_thinking or "")
    print("Saved edited image ->", edit_path)

    # 3) Image understanding: explain structure of /home/haoming/Bagel/vilex_poster.png
    poster_path = "/home/haoming/Bagel/vilex_poster.png"
    try:
        poster_img = Image.open(poster_path)
        understanding = image_understanding(poster_img, "Explain the structure of this poster", show_thinking=False)
        out_txt = os.path.join(out_dir, "vilex_poster_structure.txt")
        with open(out_txt, "w") as fh:
            fh.write(understanding or "")
        print("Saved poster understanding ->", out_txt)
    except Exception as e:
        print("Image understanding failed for", poster_path, ":", e)
