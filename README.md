Training is mainly done with bash scripts/vilex.sh which runs train/pretrain_vilex. The model in use is modeling/Bagel/bagel_vilex, and it is using dataset from data/vilex_dataset. The dataset file have a evaluation script with it to visualize attention masks and stuff. Evaluation (in progress) is currently done through app_vilex.py and test_vilex_reconstruction.py.
